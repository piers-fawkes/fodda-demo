...
app.use((req, res, next) => {
  ...
  if (signature) {
    ...
    if (!secret) {
      console.warn("⚠️ Received X-Fodda-Signature but FODDA_MCP_SECRET is not set.");
      return next();
    }
    ...
    // Timing safe compare? (Should use crypto.timingSafeEqual but requires Buffers)
    // For implementation simplicity here using standard compare but acknowledging timing attack risk is low for this specific internal use case
    if (signature !== expected) {
      console.error(`❌ Invalid HMAC. Got: ${signature}`);
...
app.use((req, res, next) => {
  ...
  // Hook into response to calculate duration
  const originalJson = res.json;
  res.json = function (body) {
    const duration = Date.now() - start;
    if (req.headers['x-fodda-profiling'] === 'true') {
      res.setHeader('X-Fodda-Latency', `${duration}ms`);
      // We can also inject into body.meta if it exists and is an object
      if (body && typeof body === 'object' && !Array.isArray(body)) {
        // Careful not to mutate if sealed, but usually safe in Express
        // If body.meta exists, add latency
...
const FODDA_API_URL = "https://fodda-contextual-demo-p3uz7zw7ja-uw.a.run.app";
...
console.log(`[Config] AIRTABLE_PAT Loaded: ${AIRTABLE_PAT ? 'YES (' + AIRTABLE_PAT.substring(0, 4) + '...)' : 'NO'}`);
...
app.post(["/api/query", "/api/query/"], async (req, res) => {
  ...
  try {
    ...
    const simulationMode = req.headers['x-fodda-simulation-mode'] as string;

    // Simulation Mode Interception
    if (simulationMode === 'gemini_echo') {
      console.log(`[${requestId}] [Simulation] Intercepting request with gemini_echo mode.`);
      const effectiveGraphId = req.body.vertical || req.body.graphId || "simulation";
      ...
      // Mock Response Data
      const mockData = {
        answer: "This is a SIMULATED response from the Fodda API (Gemini Echo Mode). No upstream providers were called.",
        evidence: [
          {
            id: "sim-1",
            title: "Simulated Evidence Node",
            text: "This node was generated by the simulation harness to verify tool invocation flow without external calls."
          }
        ],
        meta: {
          usage,
          simulation: { active: true, type: 'gemini_echo' },
...
app.post("/v1/psfk/overview", async (req, res) => {
  ...
  try {
    // 1. Auth Check (Premium / Public)
    // Premium calls include X-API-Key. Public calls are allowed but tracked anonymously.
    const apiKey = req.headers['x-api-key'] as string;
    if (!apiKey) {
      console.log(`[${requestId}] [Auth] Anonymous Overview request (Public Access)`);
    } else {
      // Optional: Logic to verify premium key and link to account
      console.log(`[${requestId}] [Auth] Premium Overview request: ${apiKey.substring(0, 8)}...`);
    }
