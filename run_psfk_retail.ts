...
async function run() {
    let md = `# PSFK Retail Graph QA Validation (Post-Update)\n\n`;
    ...
    for (const q of questions) {
        ...
        try {
            ...
            if (json.rows && json.rows.length > 0) {
                ...
                md += `**Result**: NO DATA. No matches found.\n`;
            }
        } catch (e: any) {
    ...
    const outFile = 'PSFK_Retail_QA_Results_Updated.md';
