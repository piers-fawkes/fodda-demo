                    ...
                    {articles.map(a => (
                      <div key={a.articleId} id={`article-${a.articleId}`} className={`p-4 rounded-lg border transition-all duration-500 ${highlightedItem?.type === 'article' && highlightedItem?.id === a.articleId ? 'bg-purple-50 border-fodda-accent shadow-md ring-1 ring-fodda-accent/20' : 'bg-white border-stone-200 hover:border-fodda-accent/30'}`}>
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <a href={a.sourceUrl} target="_blank" rel="noopener noreferrer" className="text-[10px] text-fodda-accent font-bold hover:underline flex items-center">
                            Source
                        ...
                        <p className="text-xs text-stone-600 italic bg-stone-50 p-2.5 rounded border border-stone-100 leading-relaxed">{a.snippet || "Reference snippet unavailable."}</p>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div className="px-6 py-4 mt-auto border-t border-stone-100 bg-stone-50/30 shrink-0">
          <div className="relative group">
            <p className="text-[9px] text-stone-400/80 leading-tight font-medium italic pr-7">
              Grounded, traceable AI demo. Powered by curated expert context.
            </p>
            <button 
              onClick={onOpenAbout}
              className="absolute top-0 right-0 p-1 text-fodda-accent/70 hover:text-fodda-accent rounded-full transition-all"
            >
