...
async function testFormula(label, filter) {
    ...
    if (data.records) {
        data.records.forEach(r => {
            console.log(`  - Key: ${r.fields['API Key']} (Account: ${JSON.stringify(r.fields.Account)})`);
        });
    }
}

async function run() {
    const accountId = 'recZPOi3zbhm2MyWi'; // The account for sk_live_7dcf37c0...
    await testFormula('Direct comparison', `AND({Account} = '${accountId}', {API Key Status} = 'Active')`);
    await testFormula('FIND', `AND(FIND('${accountId}', {Account}), {API Key Status} = 'Active')`);
